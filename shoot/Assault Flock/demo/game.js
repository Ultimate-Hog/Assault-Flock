...
function updateBirds(dt) {
  ...
  alive
    ...
    .forEach((bird, slotIdx) => {
      ...
      if (state.formation === 'flying_v' && !isLeader && bird.hp < bird.maxHp && bird.stamina > 20) {
        const leader = alive.find(b => b.formationSlot === 0);
        const leaderQuality = leader
          ? (leader.stamina / 100) * (leader.hp / leader.maxHp)
          : 0.5;
        const depthRate = slotIdx >= 5 ? 1.0 : slotIdx >= 3 ? 0.65 : 0.35;
        // Beach Screamer adjacency amplifier
        const beachNearby = alive.some(b =>
          b !== bird && b.species === 'beach_screamer' &&
          Math.abs(b.formationSlot - slotIdx) <= 1
        );
        const screamerMult = beachNearby ? 1.5 : 1.0;
        const hotRate = bird.maxHp * 0.004 * depthRate * leaderQuality * screamerMult;
        const heal = hotRate * dt;
        bird.hp = Math.min(bird.maxHp, bird.hp + heal);
        bird.slipstreamHot = hotRate;
        state.totalSlipstreamHealing = (state.totalSlipstreamHealing || 0) + heal;
      }
    ...
    if (p.owner === 'bird') {
      ...
      for (const b of state.birds) {
        ...
        if (dist(p.x, p.y, b.x, b.y) < BIRD_R + PROJ_R) {
          ...
          // Formation armor — slot-depth shelter (Flying V only)
          if (state.formation === 'flying_v' && b.formationSlot > 0) {
            const totalBirds = state.birds.length;
            const aliveBirds = state.birds.filter(x => x.alive).length;
            const cohesion   = aliveBirds / totalBirds;
            const slot = b.formationSlot;
            const baseArmor = slot >= 5 ? 0.18 : slot >= 3 ? 0.12 : 0.06;
            dmg *= (1 - baseArmor * cohesion);
          }

          // Angry Honker aura absorbs damage for nearby allies
          const nearbyHonkers = state.birds.filter(hk =>
            hk.alive && hk !== b && hk.species === 'angry_honker' &&
            dist(hk.x, hk.y, b.x, b.y) < 85
          );
          if (nearbyHonkers.length >= 2) {
            dmg *= 0.62; // double-honker overlap
          } else if (nearbyHonkers.length === 1) {
            dmg *= 0.72;
          }

          // Feathered Loiterer screen: adjacent Loiterer absorbs a share
          const loitererScreen = state.birds.find(fl =>
            fl.alive && fl !== b && fl.species === 'feathered_loiter' &&
            Math.abs(fl.formationSlot - b.formationSlot) <= 1 &&
            dist(fl.x, fl.y, b.x, b.y) < 70
          );
          if (loitererScreen) dmg *= 0.85;
...
function drawBirds() {
  state.birds.forEach(bird => {
    ...
    if (state.formation === 'flying_v' && !isLeader) {
      ...
      // Soft green pulse when actively healing
      if (bird.slipstreamHot > 0 && bird.hp < bird.maxHp) {
        const intensity = Math.min(1, bird.slipstreamHot / (bird.maxHp * 0.004)) * 0.35;
        ctx.strokeStyle = `rgba(60, 200, 120, ${intensity})`;
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(bird.x, bird.y, BIRD_R + 6, Math.PI * 0.25, Math.PI * 1.75);
        ctx.stroke();
      }
    }

    // Armor arc — shown when formation armor is active
    if (state.formation === 'flying_v' && bird.formationSlot > 0) {
      const totalBirds = state.birds.length;
      const aliveBirds = state.birds.filter(x => x.alive).length;
      const cohesion   = aliveBirds / totalBirds;
      const slot = bird.formationSlot;
      const baseArmor = slot >= 5 ? 0.18 : slot >= 3 ? 0.12 : 0.06;
      const armorVal   = baseArmor * cohesion;
      if (armorVal > 0.03) {
        const a = armorVal * 1.4;
        ctx.strokeStyle = `rgba(200, 160, 50, ${Math.min(a, 0.50)})`;
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(bird.x, bird.y, BIRD_R + 3, -Math.PI * 0.4, Math.PI * 0.4);
        ctx.stroke();
      }
    }
...
const FORMATION_DESCS = {
  flying_v:    'Classic wedge. Trailing birds get slipstream stamina recovery, passive HP regen, and formation armor (up to 18% reduction at deep rear). Leader tires faster and takes full damage. Weak to flanking.',
  loose_swarm: 'No fixed positions. High individual evasion. Difficult to AoE. No slipstream bonuses, no armor. Calls execute less predictably.',
};
...
function showDebrief() {
  ...
  if (statsEl) {
    statsEl.innerHTML = `
      ...
      <div class="stat-box">
        <div class="stat-val">${Math.round(state.totalSlipstreamHealing || 0)}</div>
        <div class="stat-name">SLIPSTREAM HEALED</div>
      </div>
    `;
  }
...
function buildEventLines() {
  ...
  const hotTotal = Math.round(state.totalSlipstreamHealing || 0);
  if (hotTotal >= 30) {
    lines.push(`<div class="event-line"><span class="hi">${hotTotal} HP</span> recovered via slipstream — rear position discipline paid off.</div>`);
  } else if (hotTotal > 0 && hotTotal < 10) {
    lines.push(`<div class="event-line">Slipstream healed only <span class="warn">${hotTotal} HP</span>. Rotate damaged birds to the rear of the V to get more recovery time.</div>`);
  }
